<div class="tf-buttons-wrapper">
    ...
    @endif
    @if($showReset)
        <x-tall-button size="sm" type="reset" wire:click.prevent="resetFormData"
                       :text="trans(config('tall-forms.reset'))" color="warning"/>
    @endif
    @if($showGoBack)
        <x-tall-button size="sm" wire:click.prevent="saveAndGoBack"
                       color="secondary">{{ $saveBackBtnTxt ?? trans(config('tall-forms.save-go-back')) }}</x-tall-button>
    @endif
    @if($showSave)
        <span x-data="{ false }"
        ...
        <x-tall-button size="sm" type="submit" wire:click.prevent="saveAndStay" wire:loading.attr="disabled"
                       color="primary">
