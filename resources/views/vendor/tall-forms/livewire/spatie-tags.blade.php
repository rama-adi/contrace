<div class="w-full">
    ...
    <div x-data @tags-search-input-{{ $field['name'] }}-focus.window="$refs.searchInput.focus()"
         class="flex flex-1 flex-wrap bg-white border rounded pl-2 pr-4 pt-2 pb-1">
        ...
        @endforeach
        <input autofocus x-ref="searchInput" wire:model.debounce.500ms="search"
               wire:keydown.space.prevent="addFromSearch" name="search"
    ...
    <div class="flex items-center py-2">
        @foreach($options as $option)
            <button wire:click.prevent="addFromOptions('{{$option}}')" type="button">
                <span
                    class="{{ $color }} inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 whitespace-no-wrap">
                    {{ $option ?? null }}
                </span>
            </button>
        @endforeach
