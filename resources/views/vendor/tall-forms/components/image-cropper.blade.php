    ...
    <div x-data="imageData{{ md5($field->name) }}()" x-init="initCroppie()" x-cloak>
        ...
        <div x-show="!showCroppie && !hasImage">
            ...
            <div class="flex flex-col items-center justify-center">
                <svg class="tf-cropper-icon " fill="none" stroke="currentColor" viewBox="0 0 24 24"
            ...
            <div class="tf-cropper-modal">
                <div>
                    ...
                    <div class="flex justify-center items-center gap-2">
                        <button type="button" class="tf-cropper-delete"
                                x-on:click.prevent="swap()">@lang(config('tall-forms.delete'))</button>
                        <button type="button" class="tf-cropper-save"
                                x-on:click.prevent="saveCroppie()">@lang(config('tall-forms.save'))</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="!showCroppie && hasImage" class="relative {{ $field->thumbnail }}">
            <div class="tf-cropper-btns-root">
                <div class="tf-cropper-btns-wrapper">
                    <button type="button" class="tf-cropper-swap" x-on:click.prevent="swap()">
                        <x-tall-svg :path="config('tall-forms.trash-icon')" class="h-6 w-6"/>
                    </button>
                    <button type="button" class="tf-cropper-edit" x-on:click.prevent="edit()">
                        <x-tall-svg :path="config('tall-forms.edit-icon')" class="h-6 w-6"/>
                    </button>
                </div>
            </div>
